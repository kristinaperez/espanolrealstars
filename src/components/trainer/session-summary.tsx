...
export function PremiumLock({ nextLesson }: { nextLesson?: number }) {
  const { state } = useProgress();
  const { config: telegramConfig, ready: telegramReady } = useTelegram();
  return (
    <div className="rounded-3xl border-2 border-primary/30 bg-primary/8 p-7 text-center sm:p-10">
      <p className="text-5xl">🔒</p>
      <h2 className="mt-4 text-2xl font-extrabold tracking-tight sm:text-3xl">
        Вы прошли бесплатную часть курса
      </h2>
      <p className="mx-auto mt-3 max-w-xl text-base text-muted">
        Откройте все 45 уроков, полную систему повторения, экзамены и сертификат. Разовая оплата — {""}
        <span className="font-bold text-foreground">15 €</span>
        {telegramReady && telegramConfig.enabled ? (
          <>
            {" "}
            или <span className="font-bold text-foreground">{telegramConfig.starsPrice} ⭐</span> Telegram Stars
          </>
        ) : null}
        , без подписки и автоплатежей.
      </p>
      ...
      <div className="mx-auto mt-6 flex max-w-md flex-col gap-3 sm:flex-row sm:justify-center">
        <Link
          ...
          className="inline-flex h-14 items-center justify-center rounded-2xl bg-primary px-7 text-base font-semibold text-primary-contrast shadow-[0_4px_0_0_var(--primary-strong)]"
        >
          {state.license ? "Управление доступом" : "Ввести лицензионный ключ"}
        </Link>
        {nextLesson ? (
          <Link
